...
class NavigationManager extends ChangeNotifier {
  ...
  bool get isSpeaking => _speechCoordinator.isSpeaking;
  ...
  void dispose() {
    _posSub?.cancel();
    _compassSub?.cancel();
    _speechCoordinator.stop();
    super.dispose();
  }

  Future<void> _initTts() async {
    await _speechCoordinator.init();
    _speechCoordinator.onSpeakingStarted = () => notifyListeners();
    _speechCoordinator.onSpeakingStopped = () => notifyListeners();
  }
  ...
  Future<void> startLiveTracking() async {
    ...
    // Timer to guarantee periodic reassurance checks even if stationary (since distanceFilter ignores <2m movement)
  ...
  void _periodicReassurance() {
    ...
    final now = DateTime.now();
    // Every 8 seconds
    if (now.difference(_lastPeriodicSpeakAt) > const Duration(seconds: 8)) {
  ...
  Future<void> _speak(String text, {bool force = false}) async {
    ...
    // Speculatively update timer to prevent re-entrancy from fast _periodicTimer
    // while we are waiting for TTS to synthesize and complete.
    DateTime oldTimer = _lastPeriodicSpeakAt;
    _lastPeriodicSpeakAt = DateTime.now();

    bool spoken = await _speechCoordinator.handleNavigationInstruction(text, force: force);
    
    if (spoken) {
      _lastSpoken = text;
    } else {
      // It was blocked by object detection, revert the timer so it tries again next second
      _lastPeriodicSpeakAt = oldTimer;
    }
  }
